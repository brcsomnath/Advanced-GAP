...
filterX = checkOut:size(2)
filterY = checkOut:size(3)
...
errorTensor = torch.Tensor(1+filterXY,#classes):zero()
...
for c=1,#classes do
	...
	for t=1,classSize[c]/batchSize do
		...
		errorTensor[filterXY+1][classTarget] = errorTensor[filterXY+1][classTarget] + error_
		for f=1,filterXY do
			collectgarbage()
			local filter_masked = torch.Tensor(filters:size()):copy(filters)
			local x = 1+torch.floor(f/filterX)
			local y = f%filterX
			if y==0 then
				y=filterX
				x = x-1
...
torch.save('modifiedGAP2_errTensor.t7',errorTensor)
