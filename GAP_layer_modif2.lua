...
WeightsTensor = torch.load('modifiedGAP2_errTensor.t7')
modifiedWeightsTensor = torch.Tensor(WeightsTensor:size()):copy(WeightsTensor)
for i=1,WeightsTensor:size(1) do
    modifiedWeightsTensor[{{i},{}}] = modifiedWeightsTensor[{{i},{}}] - modifiedWeightsTensor[{{17},{}}]
end
print(modifiedWeightsTensor)
...
function WeightedMean(A,target,modifiedWeightsTensor)
    weights = modifiedWeightsTensor[{{},{target}}]
    weights2 = torch.Tensor(weights:size(1)-1,weights:size(2))
    for i=1,weights2:size(1) do
        weights2[i] = weights[i]
    end
    max = torch.abs(weights2):max()
    weights2:div(max)
    weights2:reshape(A:size());
    local res = torch.Tensor(A:size()):fill(0);
    res:addcmul(A,weights2)
    return res:sum()/(A:size(1)*A:size(2))
end
    	...
    	local currentError = 0;
      	for t1 = 1,batchSize do
        	...
        	for filter_iter=1,numFilters do
                ...
                filter_output[t1][filter_iter] = WeightedMean(temp_out[filter_iter],target[t1],modifiedWeightsTensor)
        	end
			--print(t1)
        end
    ...
    if(trSize%batchSize ~=0) then
	    ...
	    for t1=1,(trSize%batchSize) do
        	...
        	for filter_iter=1,numFilters do
                ...
                residualFilter_output[t1][filter_iter] = WeightedMean(temp_out[filter_iter],residualTarget[t1],modifiedWeightsTensor)
        	end
		end
...
torch.save('modif2_gapWeights_MNIST2.t7',model2)
